...
from backend.features.instruments.instrument_data_service import InstrumentDataService
...
@router.get("/get_data_from_yfinance/{ticker}")
def download_data_from_yfinance(*,
    ticker: str,
    session: Session = Depends(get_session) ):    
    return InstrumentDataService(session).add_instrument_data(ticker)